...
app = typer.Typer(help="Collect papers from OpenAlex and generate pipeline outputs.")
console = Console()
...
def main(
    ...
    max_results: int = typer.Option(100, help="Maximum number of OpenAlex works to collect"),
    db_path: str = typer.Option("academic_papers.db", help="SQLite database path."),
) -> None:
    store = SQLiteStore(db_path=db_path)
    store.initialize()
    console.print(f"[bold green]Collecting OpenAlex papers [/bold green]: {query}")
    result = collect_openalex(query=query, max_results=max_results, store=store)
